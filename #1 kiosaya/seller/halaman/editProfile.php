      ...
      <form class="" action="halaman/updateProfile.php" method="post">
        <table class="table" style="widht : 100%;">
                <tr>
                    <td>Nama Lengkap</td>
                    <td>:</td>
                    <td><input type="hidden" name="profileId" value="<?php echo $rowProfile[0]['id']; ?>">
												<input class="form-control" type="text" name="name" value="<?php echo $rowProfile[0]['name']; ?>">
										</td>
                </tr>
                <tr>
          <td>Alamat</td>
                    <td>:</td>
                    <td><input class="form-control" type="text" name="address" value="<?php echo $rowProfile[0]['address']; ?>"></td>
                </tr>
								<tr>
                    <td>Propinsi</td>
                    <td>:</td>
										...
										$field   = "id,name";
										$order  = "name";
										$where = "id=".$rowProfile[0]['provinceId'];
										$on="";
										$db->select($table,$field,$on,$where,$order);
										$result = $db->getResult();
										$decode = json_decode($result,true);
										$rowProvince = $decode['post'];?>
                    <td><select name='provinces' id="provinces" class="form-control">
										<option value="<?php echo $rowProvince[0]['id']?>"><?php echo $rowProvince[0]['name'];?></option>
						        <?php
										$where = "id !=".$rowProfile[0]['provinceId'];
										$db->select($table,$field,$on,$where,$order);
										$result = $db->getResult();
										$decode = json_decode($result,true);
										$rowProvince = $decode['post'];
						        for ($r = 0; $r < count ($rowProvince);$r++){
						          ?>
						          <option value='<?php echo $rowProvince[$r]['id']?>'><?php echo $rowProvince[$r]['name']?></option>
						        <?php }
						        ?>
										</select></td>
                </tr>
                <tr>
                    ...
                    <td><select name='city' id="city" class="form-control">
												<option value="<?php echo $rowProfile[0]['cityId']?>"><?php echo $rowCity[0]['name']?></option>
										<?php
											$where = "id !=".$rowProfile[0]['cityId']." and province_id=".$rowProfile[0]['provinceId'];
											$db->select($table,$field,$on,$where,$order);
											$result = $db->getResult();
											$decode = json_decode($result,true);
											$rowProvince = $decode['post'];
                ...
                <tr>
                    <td>No. Telepon</td>
                    <td>:</td>
                    <td><input class="form-control" type="text" name="phone" value="<?php echo $rowProfile[0]['phone']; ?>"></td>
                </tr>
                <tr>
                  <td colspan="3">
