<?php
	...
	foreach ($rowCampaign as $keyCampaign => $valueCampaign) {
		if ($contractId == null) {
			$contractId = $valueCampaign['contractId'];
		}else {
			$contractId .= " , ".$valueCampaign['contractId'];
		}
	}
	...
	$field = '*';
	$on = '';
	$where = 'userId='.$userId.' and (id) in ('.$contractId.')';
	$group ='';
	...
	<div class="tableCtr">
		<table class="table table-striped table-bordered">
			...
			foreach ($rowCtr as $keyCtr => $valueCtr) {
				//echo $userId;
				$table = 'campaign';
				$field = 'count(adsId) as totalAds';
				$on = '';
				$where = 'contractId='.$valueCtr['id'];
			...
			<tbody>
				...
				<td class="centerAlign">
					<div class="btn-group">
					<button class="_play_camp btn btn-warning btn-sm detailAds" data-id="<?php echo $valueCtr['id']?>"><i style="color : white" class="fa fa-buysellads" aria-hidden="true" ></i> Ads (<?php echo $rowAds[0]['totalAds']; ?>)</button>
