...
//buat koneksi
  class configEcom extends PDO{
    ...
    public function __construct(){
      ...
      $this->engine   = 'mysql';
      $this->host     = 'localhost';
      $this->database = 'id3880635_ecommerce';
      $this->user     = 'id3880635_ecommerce';
      $this->pass     = 'kiosaya';
   ...
   */
       ...
       /*
      * Auto Delete records from the database.
      */
       public function autoDelete($table,$where=null)
       {
               $command = 'DELETE FROM '.$table;
               ...
               $test = implode(' AND ',$list);
               $explode = explode('>=',$test);
               $command .= ' WHERE '.'DATEDIFF(CURDATE(),'.$explode[0].")".">=".$explode[1];
               $test = $explode[1];
               $json = "{".substr($parameter,1)."}";
               $param = json_decode($json,true);
